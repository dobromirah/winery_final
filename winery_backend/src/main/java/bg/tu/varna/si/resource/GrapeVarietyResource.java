...
public class GrapeVarietyResource {
    ...
    public GrapeVarietyResponseDTO create(GrapeVarietyCreateDTO dto) {
        ...
        if (repository.existsByName(name))
            throw new WebApplicationException("Variety with this name already exists", 400);

        if (dto.yieldLitersPerKg <= 0)
            throw new WebApplicationException("yieldLitersPerKg must be > 0", 400);

        if (dto.criticalMinKg < 0)
            throw new WebApplicationException("criticalMinKg must be >= 0", 400);
